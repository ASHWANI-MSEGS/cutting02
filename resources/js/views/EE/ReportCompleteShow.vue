<template>
    <div class="relative" >
       ...
       <!-- Assign modals -->
        <div v-if="modalA" class="absolute left-0 right-0 z-40 h-48 ml-auto mr-auto bg-white rounded-lg shadow top-2 sm:w-2/3 lg:w-2/4 xl:w-1/3">
            <form @submit.prevent="approve">
                ...
                <div class="flex justify-center mt-12 text-xl font-bold text-textblueform ">Road Cutting Completion Verified & Refund</div>


                <div class="flex items-center justify-center pt-2 ">
                    <button class="px-4 py-2 text-sm font-bold text-white bg-green-500 rounded hover:bg-green-400" >Verified for Refund</button>
        ...
        <div v-else class="flex flex-col md:mt-6 md:ml-6">
                ...
                <div class="flex flex-row items-center justify-between h-24 mt-3 bg-white border-b-4 border-dashed rounded-t-lg rounded-b-lg sm:w-5/5 lg:w-1/2">
                    <div class="flex flex-col ml-8">
                        ...
                        <p class="text-lg font-medium">{{ report.form_memo }}</p>
                    </div>
                ...
                <div class="h-auto bg-white border-gray-200 rounded-t-lg rounded-b-lg sm:w-5/5 lg:w-1/2">

                    <div class="flex justify-start px-1 py-2 ml-0 md:ml-8">

                        <div class="grid grid-cols-2 gap-0 ml-0 md:gap-2 md:ml-4">
                                <p class="py-1">Name</p>
                                ...
                                <template v-if="form.water_id === 0">
                                    <p class="py-1">: Connection Thar (Tui) </p>
                                </template>
                                <template v-else-if="form.water_id === 1">
                                    <p class="py-1">: Connection Hlui (Tui)</p>
                                </template>
                                <template v-else>
                                    <p class="py-1">: Others </p>
                                </template>
                                <p class="py-1">Road Cut zawh ni</p>
                                <p class="py-1">: {{report.cdate}}</p>
                                <p class="py-1">A/c Name</p>
                                <p class="py-1">: {{report.ac_name}}</p>
                                <p class="py-1">A/c number</p>
                                <p class="py-1">: {{report.ac_number}}</p>
                                <p class="py-1">IFSC Code</p>
                                <p class="py-1">: {{report.ifsc}}</p>



                                <p class="py-1">In Report ni</p>
                                <p class="py-1">: {{report.created_at}}</p>



                                <a href="#"  class="px-4 py-2 my-4 text-sm font-bold text-white bg-green-500 border rounded hover:bg-green-400" @click="modalA = ! modalA">Verified for Refund</a>

                                <a href="#" class="px-4 py-2 my-4 text-sm font-bold text-center text-white bg-red-500 border rounded" @click="modal = ! modal">Report for Fine</a>
            ...
            axios.get('/api/reportCompleteShow/' + this.$route.params.id, {
                ...
                .then(response => {
                  ...
                  //  console.log(this.form.form_id);
                    ...
                    console.log(this.report.form_id);
                    this.loading = false;
                })
                .catch(error => {
                    this.loading = false;
            ...
            approve: function () {
                ...
                headers: {
                    ...
                    .then(response => {
                        ...
                        this.$router.push('/refundUnrefundList');
