<template>
                    ...
                    <div >
                        ...
                        <form @submit.prevent='registerUser'>
                            <div class="relative pt-2">
                                ...
                                <input @input="updateName()" type="text" class="w-full px-2 pt-6 pb-1 text-gray-900 border rounded-lg focus:outline-none " placeholder="Hming" v-model="user.name"  >
                            </div>
                            ...
                            <div class="relative pt-2 ">
                                ...
                                <input @input="updateEmail()" type="text" class="w-full px-2 pt-6 pb-1 text-gray-900 border rounded-lg focus:outline-none " placeholder="Email" v-model="user.email"  >
                            </div>
                            ...
                            <div class="relative pt-2 ">
                                <label  class="absolute px-2 py-2 text-xs uppercase text-textblue">Password:</label>
                                <input @input="updatePassword()" type="text" class="w-full px-2 pt-6 pb-1 text-gray-900 border rounded-lg focus:outline-none " placeholder="Password" v-model="user.password"  >
                            </div>
                            <p class="pb-4 text-sm text-red-600" v-text="errorPassword()">Error Here</p>


                            <div class="relative pt-2 ">
                                <label  class="absolute px-2 py-2 text-xs uppercase text-textblue">Phone:</label>
                                <input @input="updatePhone()" type="text" class="w-full px-2 pt-6 pb-1 text-gray-900 border rounded-lg focus:outline-none " placeholder="Phone" v-model="user.phone"  >
                            </div>
    ...
    <select v-model="user.division_id" @change="updateDivision_id()" class="w-full px-3 pt-2 pb-1 mt-1 ml-1 bg-white rounded-lg appearance-none text-textgrayform focus:outline-none">
           <option value="" class="inline-block text-textgrayform" disabled selected>Select Division</option>
           <template v-if="userDivisionId==1 || userRoleID==1 "><option value="1">Aizawl Road South</option></template>
           <template v-if="userDivisionId==2 || userRoleID==1 "><option value="2">Aizawl Road North</option></template>
           <template v-if="userDivisionId==4 || userRoleID==1 "><option value="4">Champhai Road</option></template>
           <template v-if="userDivisionId==5 || userRoleID==1 "><option value="5">Lunglei Road</option></template>
           <template v-if="userDivisionId==6 || userRoleID==1 "><option value="6">Saitual Road</option></template>
           <template v-if="userDivisionId==7 || userRoleID==1 "><option value="7">Mamit Road</option></template>
           <template v-if="userDivisionId==8 || userRoleID==1 "><option value="8">Siaha Road</option></template>
           <template v-if="userDivisionId==9 || userRoleID==1 "><option value="9">Khawzawl Road</option></template>
           <template v-if="userDivisionId==10 || userRoleID==1 "><option value="10">Kolasib Road</option></template>
           <template v-if="userDivisionId==11 || userRoleID==1 "><option value="11">Lawngtlai Road</option></template>
           <template v-if="userDivisionId==12 || userRoleID==1 "><option value="12">Serchhip Road</option></template>
           <template v-if="userDivisionId==13 || userRoleID==1 "><option value="13">Lunglei Road Div-II, Hnahthial Road</option></template>
           <template v-if="userDivisionId==14 || userRoleID==1 "><option value="14">Hmuifang Division Road</option></template>
           <template v-if="userDivisionId==15 || userRoleID==1 "><option value="15">National Highway Div-I Road</option></template>
           <template v-if="userDivisionId==16 || userRoleID==1 "><option value="16">National Highway Div-III Road</option></template>
    </select>
    </div>
...
<!-- original code end -->
                            ...
                            <div  class="relative pb-2 mb-3 border rounded-lg" >
                                <label for="user.sub_div" class="absolute px-4 py-2 text-sm font-light text-textblueform">Sub Division </label>
                                <select v-model="user.sub_div" class="w-full px-3 pt-6 mt-1 ml-1 bg-white rounded-lg appearance-none text-textgrayform focus:outline-none" :errors="errors">
                            ...
                            <template v-if="userRoleID==1">
                                <label class="cursor-pointer hover:text-gray-600"><input @input="updateRole_id()" class="hover:text=blue=400  pr-4" type="radio" value="1" v-model="user.role_id" > Super Admin</label>
                            </template>
                                <label class="cursor-pointer hover:text-gray-600"><input @input="updateRole_id()" class="hover:text-blue-400 " type="radio" value="2"  v-model="user.role_id" > EE </label>
                                <label class="cursor-pointer hover:text-gray-600"><input @input="updateRole_id()" class="hover:text-blue-400 " type="radio" value="3"  v-model="user.role_id" > SDO/JE</label>
                                </div>
                            </div>
