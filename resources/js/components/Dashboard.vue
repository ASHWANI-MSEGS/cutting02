<template>
    <div class="h-full">
         <div class="pb-16 ">
            ...
            <div class="flex h-auto mx-0 my-0 bg-white rounded-lg md:mx-8 md:my-8">

                <div class="w-1/2 pl-12 pr-40 my-16 ">
                    <p class="text-4xl font-medium leading-tight text-gray-900">Welcome to <span class="underline underline-offset-8">{{divisions[division_id]}}</span >  Dashboard
                        <br class="hidden lg:inline">
                    </p>
                    <p class="hidden mt-4 text-base text-gray-600 lg:block">
                        You can apply for road cutting permit from home, work place or around the globe
                    </p>
                </div>
                <div class="hidden w-1/2 lg:block lg:relative ">
                    <img class="absolute inset-0 object-cover object-center w-full h-full rounded-r-lg "
                        :src="bgImage" />
                </div>
            </div>

            <div class="grid h-full grid-cols-1 gap-4 mx-0 my-0 rounded-lg lg:grid-cols-2 md:mx-8 md:my-8">

                <div class="bg-white rounded-lg shadow-xl ">
                    <div class="flex justify-between px-2 py-2 mx-8">
                        <div class="flex flex-col ">
                            <p class="py-1 text-sm text-gray-600">FORM SUBMITTED</p>
                            <p class="py-1 text-xl font-semibold">{{ formSubmitTotal }}</p>
                            <p class="pb-1 mt-4 text-sm text-blueGray-400"><span class="mr-2 text-green-500"><i
                                        class="fas fa-arrow-up"></i> 3.48% </span><span class="whitespace-nowrap">Since
                                    last month</span></p>
                        </div>
                        <div>
                            <div
                                class="flex items-center justify-center w-12 h-12 mt-8 ml-4 rounded-full bg-buttonpink rounded-circle">
                                <i class="text-xl text-white far fa-chart-bar"></i>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-xl ">
                    <div class="flex justify-between px-2 py-2 mx-8">
                        <div class="flex flex-col ">
                            <p class="py-1 text-sm text-gray-600">FORM REJECTED</p>
                            <p class="py-1 text-xl font-semibold">{{ formRejectedTotal }}</p>
                            <p class="pb-1 mt-4 text-sm text-blueGray-400"><span class="mr-2 text-red-500"><i
                        ...
                        <div>
                            <div
                                class="flex items-center justify-center w-12 h-12 mt-8 ml-4 rounded-full bg-buttonpink rounded-circle">
                                <i class="text-xl text-white fa fa-ban"></i>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-xl ">
                    ...
                    <div class="flex justify-center mx-2 my-2 text-xl font-medium text-tableheader ">Amount Received
                        Summary</div>
                    <div class="flex justify-start py-2 pl-2 ml-1 md:ml-8">
                        <div class="grid grid-cols-2 gap-1 mx-1 md:gap-2 md:mx-4">
                            <p class="py-1"> Estimated Amount</p>
                            <p class="py-1 font-medium">: Rs. {{ estimateAmount }}</p>
                            <p class="py-1"> Estimated Security Deposit</p>
                            <p class="py-1 font-medium">: Rs. {{ estimateDeposit }}</p>

                            <p class="py-1">Total Estimated Amount </p>
                            <p class="py-1 font-medium">: Rs. {{ estimateTotal }}</p>
                            <p class="py-1">Total Amount Received in Bank</p>
                            <p class="py-1 font-medium">: Rs {{ paymentReceived }}</p>
                            <br>
                            <p class="py-1">
                                <router-link to="/amountDetails">
                                    <button
                                        class="px-4 py-2 font-medium text-blue-400 border border-blue-400 rounded-lg hover:text-gray-500">Monthly
                                        Details</button>
                                </router-link>
                            </p>
                            <br>
                ...
                <div class="bg-white rounded-lg shadow-xl">
                    ...
                    <div class="flex justify-center mx-2 my-2 text-xl font-medium text-tableheader">Security Deposit
                        Summary</div>
                    <div class="flex justify-start py-2 pl-2 ml-1 md:ml-8">
                        <div class="grid grid-cols-2 gap-1 mx-1 md:gap-2 md:mx-4">
                            <p class="py-1">No of Security Deposit Returned Successfully</p>
                            <p class="py-1 font-medium">: {{ depositReturnedNo }}</p>
                            <p class="py-1">No of Security Deposit to be returned</p>
                            <p class="py-1 font-medium">: {{ depositTobeReturNo }}</p>

                            <p class="py-1">Security Deposit Returned</p>
                            <p class="py-1 font-medium">: Rs. {{ depositRefunded }}</p>
                            <p class="py-1">Deposit To be Returned</p>
                            <p class="py-1 font-medium">: Rs. {{ depositTobeRefunded }}</p>

                            <p class="py-1">Total Fine received</p>
                            <p class="py-1 font-medium">: Rs. {{ depositFine }}</p>
                            <p class="py-1">SD to be returned List </p>
                            <p class="py-1">:<a :href="'api/auth/exportRefund/' + division_id"
                                    class="hover:text-gray-400"><span
                                        class="px-1 font-medium text-green-500 rounded-lg hover:text-gray-500">Download<span
                                            class="px-1 xl:py-0">(Excel)</span></span>
                                </a>
                            </p>
                ...
                <div class="bg-white rounded-lg shadow-xl">
                    ...
                    <div class="flex justify-center mx-2 my-2 text-xl font-medium text-tableheader">Local Council</div>
                    <div class="flex justify-start py-2 pl-2 ml-1 md:ml-8">
                        <div class="grid grid-cols-2 gap-1 mx-1 md:gap-2 md:mx-4">
                            <template v-if="division_id == 1 || division_id == 2">
                                <p class="py-1">No. of Local Council in Aizawl Road South</p>
                                <p class="py-1 font-medium">: {{ localcouncilSouthno }}</p>
                                <p class="py-1">No. of Local Council in Aizawl Road North</p>
                                <p class="py-1 font-medium">: {{ localcouncilNorthno }}</p>
                            </template>

                            <template v-else>
                                <p class="py-1">No. of Local Council </p>
                                <p class="py-1 font-medium">: {{ localcouncilNo }}</p>
                                <!-- <p class="py-1">dfggsdfgsdg</p>
                                <p class="py-1 font-medium">sdfgsdfgdf</p> -->
                                <!-- <br> -->
                            </template>
                            <p class="py-1">Add New Local Council</p>
                            <p class="py-1 font-medium">:
                                <router-link to="/localcouncilAdd">
                                    <button  class="text-blue-400 hover:text-gray-400">Add </button>
                                </router-link>
                            </p>
                            <p class="py-1">Local Council List</p>
                            <p  class="py-1 font-medium">:
                                <router-link to="/localcouncilList">
                                    <button  class="text-blue-400 hover:text-gray-400">View </button>
                                </router-link>
                            </p>
                            <br>
                            <br>
                ...
                <div class="bg-white rounded-lg shadow-xl">
                    <div class="flex justify-center mt-4">
                        <svg width="80px" height="80px" viewBox="0 0 80 80" version="1.1"
                            xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                            <title>7EC39253-3F7D-4D08-AF09-EE28D42AEEB3</title>
                            <g id="Others" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                <g id="PWD" transform="translate(-1085.000000, -3099.000000)">
                                    <g id="Group-4" transform="translate(1085.000000, 3099.000000)">
                                        <g id="worker" transform="translate(10.000000, 3.000000)" fill-rule="nonzero">
                                            <path
                                                d="M59,62.1820842 L59,71 L50,71 L50,49 C55.298,51.2786614 59,56.442041 59,62.1820842 Z"
                                                id="Path" fill="#ADC5F6"></path>
                                            <path
                    ...
                    <div class="flex justify-center mx-2 my-2 text-xl font-medium text-tableheader">Site Engineer</div>
                    <div class="flex justify-start py-2 pl-2 ml-1 md:ml-8">
                        <div class="grid grid-cols-2 gap-1 mx-1 md:gap-2 md:mx-4">
                            <template v-if="division_id == 1 || division_id == 2">
                                <p class="py-1">No. of Site Engineer in Aizawl Road South</p>
                                <p class="py-1 font-medium">: {{ engineerSouthno }}</p>
                                <p class="py-1">No. of Site Engineer in Aizawl Road North</p>
                                <p class="py-1 font-medium">: {{ engineerNorthno }}</p>
                            </template>
                            <template v-else>
                                <p class="py-1">No. of Site Engineer </p>
                                <p class="py-1 font-medium">: {{ engineerNo }}</p>
                            </template>

                            <p class="py-1">Add Engineer</p>
                            <p class="py-1 font-medium">:
                                <router-link to="/engineerAdd">
                                    <button class="text-blue-400 hover:text-gray-400">Add </button>
                                </router-link>
                            </p>
                            <p class="py-1">Engineer List</p>

                            <p class="py-1 font-medium">:
                                <router-link to="/engineerList">
                                    <button class="text-blue-400 hover:text-gray-400">View</button>
                                </router-link>
                            </p>
                        </div>
...
// console.log(id,"            ig");
        axios.get('/api/dashboard/' + this.division_id, {
            ...
            .then(response => {
                ...
                // console.log("geting the data of the list".this.formSubmitTotalList);
                this.formSubmitTotal = response.data.formSubmitTotal;

                this.formRejectedTotal = response.data.formRejectedTotal;

                this.estimateAmount = response.data.estimateAmount;
                this.estimateDeposit = response.data.estimateDeposit;
                this.estimateTotal = response.data.estimateTotal;
                this.paymentReceived = response.data.paymentReceived;

                this.localcouncilSouthno = response.data.localcouncilSouthno;
                this.localcouncilNorthno = response.data.localcouncilNorthno;
                this.localcouncilNo = response.data.localcouncilNo;

                this.engineerSouthno = response.data.engineerSouthno;
                this.engineerNorthno = response.data.engineerNorthno;
                this.engineerNo = response.data.engineerNo;


                this.depositRefunded = response.data.depositRefunded,

                    this.depositTobeRefunded = response.data.depositTobeRefunded,

                    this.depositFine = response.data.depositFine,

                    this.depositReturnedNo = response.data.depositReturnedNo,

                    this.depositTobeReturNo = response.data.depositTobeReturNo,

                    // this.division_id = this.$store.getters.currentUser.division_id,
