        ...
        <div v-else>
            ...
            <div class="flex justify-between">
                ...
                <div class="relative">
                    ...
                    <a href="#" v-if="form.status === 0" class="px-4 py-2 border border-green-500 rounded text-sm font-bold text-green-500" @click="modalA = ! modalA">Send to EE for Verification</a>

                    <div v-if="modalA" class="absolute bg-blue-600 text-white rounded-lg z-20 p-8 w-96 left mt-2 mr-6">
                        <p>Are you sure you want to Send this form for Verification?</p>

                        <div class="flex items-center mt-6 justify-end">
                            <button class="text-white pr-4" @click="modalA = ! modalA">Cancel</button>
                            <button class="px-4 py-2 bg-green-500 rounded  text-sm font-bold text-white" @click="approve">Send to EE for Verification</button>
                        </div>
                    </div>

                    <a href="#" class="px-4 py-2 border border-red-500 rounded text-sm font-bold text-red-500" @click="modal = ! modal">Reject</a>

                    <div v-if="modal" class="absolute bg-blue-900 text-white rounded-lg z-20 p-8 w-64 right-0 mt-2 mr-6">
                        <p>Are you sure you want to Reject this form?</p>

                        <div class="flex items-center mt-6 justify-end">
                            <button class="text-white pr-4" @click="modal = ! modal">Cancel</button>
                            <button class="px-4 py-2 bg-red-500 rounded text-sm font-bold text-white" @click="destroy">Reject</button>
                        </div>
                    </div>
                </div>
                ...
                <p><span class="pt-6 text-gray-600 font-bold uppercase text-sm">Memo:</span>
                <span class="pl-2 text-xl">{{ form.memo }}</span>
                </p>
                <br>
            
                <p><span class="pt-6 text-gray-600 font-bold uppercase text-sm">Name</span>
                <span class="pl-2 text-xl">{{ form.name }}</span>
                </p>
                <br>

                <p><span class="pt-6 text-gray-600 font-bold uppercase text-sm">Email</span>
                <span class="pt-2 text-blue-400">{{ form.email }}</span>
                </p>
                 <br>

                <p><span class="pt-6 text-gray-600 font-bold uppercase text-sm">Phone Number:</span>
                <span class="pt-2 text-blue-400">{{ form.phone }}</span>
                </p>
                 <br>

                <p><span class="pt-6 text-gray-600 font-bold uppercase text-sm">Address:</span>
                <span class="pt-2 text-blue-400">{{ form.location }}</span>
                </p>
                 <br>


                <p><span class="pt-6 text-gray-600 font-bold uppercase text-sm">Length of the road to be cut:</span>
                <span class="pt-2 text-xl">{{ form.length }}</span>
                 </p>
                  <br>

                <p> <span class="pt-6 text-gray-600 font-bold uppercase text-sm">Tui Connection: </span>
                <template v-if="form.water_type === 0">
                <span class="pt-2 text-blue-400">New Connection</span>
                </template>

                <template v-else>
                <span class="pt-2 text-blue-400">Old Connection</span>
                </template>
                </p>
                 <br>

                <p><span class="pt-6 text-gray-600 font-bold uppercase text-sm">Road Type: </span>
                <template v-if="form.water_type === 0">
                <span class="pt-2 text-blue-400"> Flexible Pavement (Alkatra luan):</span>
                </template>
                
                <template v-else>
                <span class="pt-2 text-blue-400">Rigid Pavement (Concrete-a siam)</span>
                </template>
                </p>
                 <br>
                ...
                <template v-if="form.status === 0">
                        <p class="pt-6 text-gray-600 font-bold uppercase text-sm pr-2">Status: <span class="bg-red-400 rounded text-white px-1 py-1 ">UNAPPROVED</span></p>
                </template>
                <template v-else>
                        <p class="pt-6 text-gray-600 font-bold uppercase text-sm pr-2">Status: <span class="bg-green-400 rounded text-white px-1 py-1">APPROVED</span></p>
                </template> 

                           <br>

          <button class="text-white  rounded-lg px-4 py-1" @click.prevent="printme">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            approve: function () {
                axios.patch('/api/auth/form/list2/' + this.$route.params.id)
                    .then(response => {
                        this.$router.push('/formListUnCO');
